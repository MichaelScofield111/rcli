...
pub fn process_text_verify(
    ...
    // 因为传进去以后，整个所有权都 move 到 process_text_verify 里了。
    match format {
        TextSignFormat::Blake3 => Blake3::try_new(key)?.verify(reader, sig),
        TextSignFormat::Ed25519 => Ed25519Verifier::try_new(key)?.verify(reader, sig),
    }
}
